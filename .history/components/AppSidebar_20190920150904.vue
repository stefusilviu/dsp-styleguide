<template>
  <el-menu class="side-menu" :collapse="$root.context.collapsed" :router="true" :unique-opened="true" :default-active="$nuxt.$route.path" active-text-color="#04816A">
    ...
    <menu-toggle @togglemenu="toggle" :collapse="$root.context.collapsed"></menu-toggle>

    {{$root.context.collapsed}}
...
export default {
  ...
  methods: {
    toggle() {
      this.$root.context.collapsed = !this.$root.context.collapsed
      console.log(this.$root.context.collapsed)
      
    }
  },
  mounted() {
    console.log(this.$store.state.sidebarCollapsed)
  }
};
</script>